...
namespace SistemaVentas.Presentacion.Secciones
{
    public partial class FrmSecciones : Form
    {
        ...
        private void FrmSecciones_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                DataSet ds = FSeccion.GetAll();//creamos un dataset, un contenedor de datos y hacemos una consulta con la clase FCategoria y su método GetAll
        ...
        {
            FrmAgregarSeccion add = new FrmAgregarSeccion();
            DialogResult res = add.ShowDialog();

            FrmSecciones_Load(null, null);
        }
        ...
        {
            FrmEditarSeccion frmMod = new FrmEditarSeccion();
            ...
            FrmSecciones_Load(null, null);// se cierra el anterior formulario de editar y recargamos el formularioCategoria para actualizar datos modificados
        ...
        private void btnEliminar_Click(object sender, EventArgs e)
        {
            try
            {
                    ...
                    MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == DialogResult.OK)
                {
                    ...
                    Seccion seccion = new Seccion();// instanciamo nuestra clase para rellenarle sus atributos
                    seccion.Id = Convert.ToInt32(dgvDatos.CurrentRow.Cells["Id"].Value.ToString());// para eliminar solo le pasaremos su atributo id

                    if (FSeccion.Eliminar(seccion) > 0)// Ejecutamos el procedimiento almacenado para eliminar, si se pudo eliminar... recargamos el formulario
                    {
                        FrmSecciones_Load(null, null);
                    }
                    else
