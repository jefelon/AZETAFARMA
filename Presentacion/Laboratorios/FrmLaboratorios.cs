...
namespace SistemaVentas.Presentacion.Laboratorios
{
    public partial class FrmLaboratorios : Form
    {
        ...
        private void FrmLaboratorios_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                DataSet ds = FLaboratorio.GetAll();//creamos un dataset, un contenedor de datos y hacemos una consulta con la clase FCategoria y su método GetAll
        ...
        {
            FrmAgregarLaboratorio add = new FrmAgregarLaboratorio();
            DialogResult res = add.ShowDialog();

            FrmLaboratorios_Load(null, null);
        }
        ...
        {
            FrmEditarLaboratorio frmMod = new FrmEditarLaboratorio();
            ...
            FrmLaboratorios_Load(null, null);// se cierra el anterior formulario de editar y recargamos el formularioCategoria para actualizar datos modificados
        ...
        private void btnEliminar_Click(object sender, EventArgs e)
        {
            try
            {
                    ...
                    MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == DialogResult.OK)
                {
                    ...
                    Laboratorio laboratorio = new Laboratorio();// instanciamo nuestra clase para rellenarle sus atributos
                    laboratorio.Id = Convert.ToInt32(dgvDatos.CurrentRow.Cells["Id"].Value.ToString());// para eliminar solo le pasaremos su atributo id

                    if (FLaboratorio.Eliminar(laboratorio) > 0)// Ejecutamos el procedimiento almacenado para eliminar, si se pudo eliminar... recargamos el formulario
                    {
                        FrmLaboratorios_Load(null, null);
                    }
                    else
