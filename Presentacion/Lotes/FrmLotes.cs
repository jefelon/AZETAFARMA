...
namespace SistemaVentas.Presentacion.Lotes
{
    public partial class FrmLotes : Form
    {
        ...
        private void FrmLotes_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                DataSet ds = FLote.GetAll();//creamos un dataset, un contenedor de datos y hacemos una consulta con la clase FCategoria y su método GetAll
        ...
        {
            FrmAgregarLote add = new FrmAgregarLote();
            DialogResult res = add.ShowDialog();

            FrmLotes_Load(null, null);
        }
        ...
        {
            FrmEditarLote frmMod = new FrmEditarLote();
            ...
            FrmLotes_Load(null, null);// se cierra el anterior formulario de editar y recargamos el formularioCategoria para actualizar datos modificados
        ...
        private void btnEliminar_Click(object sender, EventArgs e)
        {
            try
            {
                    ...
                    MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == DialogResult.OK)
                {
                    ...
                    Lote lote = new Lote();// instanciamo nuestra clase para rellenarle sus atributos
                    lote.Id = Convert.ToInt32(dgvDatos.CurrentRow.Cells["Id"].Value.ToString());// para eliminar solo le pasaremos su atributo id

                    if (FLote.Eliminar(lote) > 0)// Ejecutamos el procedimiento almacenado para eliminar, si se pudo eliminar... recargamos el formulario
                    {
                        FrmLotes_Load(null, null);
                    }
                    else
